...
### 🚨 커밋 전 필수 — 검사 엔진 설명 문서 갱신

엔진 동작이 바뀌면(시그널 추가·판정 규칙 변경·타임아웃 조정·측정 수치 갱신)
**커밋하기 전에 아래 셋을 함께 고친다.** 하나라도 빠지면 커밋하지 않는다.

| 대상 | 무엇을 |
| --- | --- |
| `docs/how-it-works.md` | 파이프라인·시그널 표·판정 규칙·실측 수치 |
| `bench/README.md` | 새 측정을 했다면 결과와 재현 방법 |
| 아티팩트 "아차차 검사 엔진" | 위 내용 요약 + 상단 통계 숫자 |

아티팩트 주소: https://claude.ai/code/artifact/dcbddef4-908d-4d9b-9b9d-7fd7a1fce8a0
(소스는 세션 scratchpad의 `ahchacha-engine.html`. 같은 URL로 다시 올리려면
`Artifact` 호출에 `url` 파라미터를 넘길 것 — 안 넘기면 새 아티팩트가 생긴다)

**왜 규칙으로 박아두는가**: 두 번 놓쳤다. Sprint 3 이후 아티팩트를 한동안
갱신하지 않아 "시그널 5/10", "브랜드 목록이 미검증" 같은 이미 틀린 내용이
남아 있었다. 문서가 틀리면 무엇이 동작하는지 아무도 모르게 된다.

상단 통계 숫자(동작 시그널 수, 브랜드 개수, 오탐률, 검사 소요)도 실측값으로
맞춘다. 추정치를 적지 말 것.
